...
const state = {
  ...
  activePeerId: null, stopMessages: null, stopInbox: null, stopTyping: null, stopClears: null, signUp: false,
  replyTo: null, pendingImageFile: null, inboxReady: false, typingTimer: null, typingExpiryTimer: null
};
...
async function startConversation(peerId) {
  ...
  try {
    ...
    await runTransaction(ref(db, `chatInboxes/${peerId}/${threadId}`), (current) => current || { ...summary, peerId: state.user.uid });
...
function resetComposer() { $('message-input').value = ''; $('message-input').style.height = ''; $('image-input').value = ''; state.pendingImageFile = null; clearReply(); setTyping(false); }
...
async function sendMessage(event) {
  ...
  try {
    ...
    resetComposer();
    try { await updateConversationSummaries(text || '📷 Photo', timestamp); } catch (error) { console.error('Message was sent, but its inbox summary failed:', error); showToast('Message sent. Its inbox preview will catch up when the connection returns.'); }
...
function closeActiveChat() {
  ...
  state.stopMessages = null; state.stopTyping = null; state.activeThreadId = null; state.activePeerId = null; state.messages = {}; state.typing = {}; clearReply(); closeMessageMenu();
...
async function removeConversation() {
  ...
  try { const threadId = state.activeThreadId; await remove(ref(db, `chatInboxes/${state.user.uid}/${threadId}`)); await remove(ref(db, `chatClears/${state.user.uid}/${threadId}`)); $('conversation-dialog').close(); closeActiveChat(); showToast('Conversation removed from your inbox.'); } catch (error) { showToast(`Could not remove conversation: ${error.message.replace('Firebase: ', '')}`); }
}
...
function handleInbox(snapshot) {
  ...
  state.inboxReady = true; renderConversations(); updateUnreadTitle(); markThreadRead();
}

onValue(ref(db, 'users'), (snapshot) => { const raw = snapshot.val() || {}; state.users = Object.fromEntries(Object.entries(raw).map(([uid, profile]) => [uid, { ...(profile || {}), uid }])); renderConversations(); renderPeople(); updateChatHeader(); });
onValue(ref(db, 'presence'), (snapshot) => { state.online = snapshot.val() || {}; renderPeople(); updateChatHeader(); });
onAuthStateChanged(auth, (user) => {
  state.user = user; if (state.stopInbox) state.stopInbox(); if (state.stopClears) state.stopClears(); state.inbox = {}; state.clears = {}; state.inboxReady = false;
  if (user) {
    state.stopInbox = onValue(ref(db, `chatInboxes/${user.uid}`), handleInbox);
    state.stopClears = onValue(ref(db, `chatClears/${user.uid}`), (snapshot) => { state.clears = snapshot.val() || {}; if (state.activeThreadId) renderMessages(state.messages); });
