...
window.renderFeed = (resetLimit = true) => {
    ...
    let displayPosts = window.allPosts.filter(p => {
        ...
        // ==========================================
        // V6.1 NEW FEATURE: VISIBILITY FILTERING
        // ==========================================
        if (p.visibility === 'private') {
            if (!window.currentUser) return false; // Guests can't see private posts
            if (p.authorId !== window.currentUser.uid) { // Not the author
                ...
                // If current user is not mentioned, hide it
                if (!postText.includes(mentionStr)) return false;
            }
        }
...
window.renderProfileData = (resetLimit = true) => {
    ...
    let pPosts = window.allPosts.filter(p => {
        ...
        // ==========================================
        // V6.1 NEW FEATURE: PROFILE VISIBILITY FILTERING
        // ==========================================
        if (p.visibility === 'private') {
            if (!window.currentUser) return false; // Guests can't see private posts
            if (p.authorId !== window.currentUser.uid) { // Not the author
...
window.generatePostHTML = function(post, prefix, filterContext) {
    ...
    const getHasReacted = (type) => (window.currentUser && rx[type] ? !!rx[type][window.currentUser.uid] : false);
    ...
    const generateCRxBtn = (c, type, icon, cId) => {
        ...
        const hasReacted = window.currentUser && cRx[type] ? !!cRx[type][window.currentUser.uid] : false;
    ...
    if(window.currentUser) {
        ...
        if(window.currentUser.uid === post.authorId) adminControls += `<button onclick="window.editPost('${post.id}')" class="text-gray-400 hover:text-blue-500 mr-2 text-xs"><i class="fa-solid fa-pen"></i></button>`;
    ...
    // ==========================================
    // V6.1 NEW FEATURE: VISIBILITY ICON IN UI
    // ==========================================
