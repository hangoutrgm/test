...
// ==========================================
// V6.1 NEW FEATURES: VISIBILITY & MENTIONS
// ==========================================
...
const setupVisibilityToggle = () => {
    const initToggle = () => {
        // Will place the eye icon right before the submit button
        const submitBtn = document.getElementById('submit-post-btn');
        if (submitBtn && !document.getElementById('visibility-toggle-btn')) {
            const eyeBtn = document.createElement('button');
            eyeBtn.id = 'visibility-toggle-btn';
            eyeBtn.innerHTML = '<i class="fas fa-eye text-blue-500"></i>';
            eyeBtn.className = 'mr-3 p-2 rounded hover:bg-gray-200 dark:hover:bg-slate-700 transition flex items-center justify-center';
            ...
            eyeBtn.addEventListener('click', (e) => {
                ...
                if (window.postVisibility === 'public') {
                    ...
                    eyeBtn.innerHTML = '<i class="fas fa-eye-slash text-gray-500"></i>';
                    eyeBtn.title = "Private Post (Only you and mentioned users)";
                    if(window.showAlert) window.showAlert("Post set to Private (Only You & Mentioned Users)");
                } else {
                    window.postVisibility = 'public';
                    eyeBtn.innerHTML = '<i class="fas fa-eye text-blue-500"></i>';
                    eyeBtn.title = "Public Post";
...
const setupMentionSystem = () => {
    const initMention = () => {
        ...
        if (!suggestionBox) {
            ...
            suggestionBox.className = 'hidden absolute z-50 bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 shadow-lg rounded-md mt-1 overflow-y-auto w-64 max-h-48 left-4';
            postInput.parentNode.style.position = 'relative';
            postInput.parentNode.appendChild(suggestionBox);
        }
        ...
        postInput.addEventListener('input', (e) => {
            ...
            // Check if user is typing a mention (e.g., "@joh")
            ...
            if (match) {
                ...
                // Suggest users closest to the typed letters
                const matchedUsers = allUsers.filter(u => u.name && u.name.toLowerCase().includes(query)).slice(0, 5);
                ...
                if (matchedUsers.length > 0) {
                    ...
                    matchedUsers.forEach(u => {
                        ...
                        item.innerHTML = `<img src="${u.pic || window.generateAvatar(u.uid || 'guest')}" class="w-6 h-6 rounded-full object-cover"> <span>${u.name}</span>`;
...
document.getElementById('submit-post-btn').addEventListener('click', async () => {
    ...
    try {
        ...
        await set(postRef, {
            ...
            visibility: window.postVisibility || 'public' // <--- V6.1 Core Addition
        });
        ...
        // <--- V6.1 Reset visibility state after posting --->
        window.postVisibility = 'public';
